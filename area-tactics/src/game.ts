...
export class GameProcessor {
  ...
  constructor(
    ...
    restoredTurnStartPositions?: Map<UnitId, Position>
  ) {
    this.game = game;
    this.unitTypes = unitTypes;
    this.features = features;
    this.influences = new InfluenceMap();
    if (restoredTurnStartPositions) {
      this.turnStartPositions = new Map(restoredTurnStartPositions);
    } else {
  ...
  getTurnStartPositions(): Map<UnitId, Position> {
    return this.turnStartPositions;
  }

  /**
   * Applies a GameEvent directly to the processor's state without validation.
   * Used on the client side to stay in sync with server-broadcast events.
   */
  applyEvent(event: GameEvent): void {
    switch (event.type) {
      case "UnitMoved": {
        const unit = this.findUnit(event.unit.unitId);
        if (unit) {
      ...
      case "UnitDamaged": {
        const unit = this.findUnit(event.unit.unitId);
        if (unit) {
          unit.energy = Math.max(0, unit.energy - event.damageToEnergy);
          unit.condition = Math.max(0, unit.condition - event.damageToCondition);
        }
        break;
      }
      case "EnergyRegenerated": {
        ...
        if (unit) {
          const ut = this.unitTypes.get(unit.typeId);
          unit.energy = Math.min(unit.energy + event.amount, ut?.maxEnergy ?? unit.energy + event.amount);
        }
        break;
      }
      case "UnitDestroyed": {
        this.game.players.get(event.unit.playerId)?.units.delete(event.unit.unitId);
        break;
      }
      case "TurnStarted": {
        this.game.currentPlayerId = event.playerId;
      ...
      case "PlayerEliminated": {
        const player = this.game.players.get(event.playerId);
      ...
      case "GameEnded":
        break;
      case "BuildOrdered": {
        const player = this.game.players.get(event.unit.playerId);
        if (player) {
          const ut = this.unitTypes.get(event.unit.typeId);
          player.units.set(event.unit.unitId, {
            id: event.unit.unitId,
            position: event.facilityPosition,
            typeId: event.unit.typeId,
            playerId: event.unit.playerId,
            energy: ut?.maxEnergy ?? 0,
            condition: ut?.maxCondition ?? 0,
            underConstruction: true,
          });
        }
        break;
      ...
      case "UnitBuilt": {
        const unit = this.findUnit(event.unit.unitId);
        if (unit) unit.underConstruction = false;
        break;
      }
      case "BuildCancelled": {
        this.game.players.get(event.unit.playerId)?.units.delete(event.unit.unitId);
  ...
  private findUnit(unitId: UnitId): Unit | undefined {
