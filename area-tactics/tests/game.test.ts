...
import { UnitMovedEvent, GameEvent } from "../src/events";
...
// ============================================================================
// GameProcessor.applyEvent
// ============================================================================

function makeApplyEventGame(): { game: Game; unitTypes: Map<string, UnitType> } {
  const uid = (n: number) => n as unknown as UnitId;
  const unitTypes = new Map<string, UnitType>();
  unitTypes.set("infantry", {
    id: "infantry",
    effectType: EffectType.Direct,
  ...
  const u1: Unit = { id: uid(1), position: createPosition(0, 0), typeId: "infantry", playerId: 1, energy: 6, condition: 10 };
  const u2: Unit = { id: uid(2), position: createPosition(5, 0), typeId: "infantry", playerId: 2, energy: 6, condition: 10 };

  const game: Game = {
    map: {
      grid: HexGrid.rect(10, 1),
      tiles: new Map(),
      bases: new Map(),
    },
    players: new Map([
      [1, { id: 1, type: PlayerType.Human, units: new Map([[uid(1), u1]]) }],
      [2, { id: 2, type: PlayerType.Human, units: new Map([[uid(2), u2]]) }],
    ]),
    currentPlayerId: 1,
    turn: 1,
  };
...
describe("GameProcessor.applyEvent", () => {
  const uid = (n: number) => n as unknown as UnitId;
  const ref = (unitId: number, playerId: number, typeId = "infantry") => ({ unitId: uid(unitId), playerId, typeId });

  it("UnitMoved — updates unit position", () => {
    const { game, unitTypes } = makeApplyEventGame();
    const proc = new GameProcessor(game, unitTypes);
    proc.applyEvent({ type: "UnitMoved", unit: ref(1, 1), position: createPosition(2, 0) });
    expect(proc.getGame().players.get(1)!.units.get(uid(1))!.position).toEqual(createPosition(2, 0));
  });

  it("UnitMoved — recalculates influences (unit moves into AoI)", () => {
    const { game, unitTypes } = makeApplyEventGame();
    const proc = new GameProcessor(game, unitTypes);
    // Initially p2 unit at (5,0) is out of p1 AoI (0-2). Move p2 into range.
    proc.applyEvent({ type: "UnitMoved", unit: ref(2, 2), position: createPosition(1, 0) });
    const influenced = proc.getInfluences().getUnitsInfluencedBy(uid(1));
    expect(influenced.has(uid(2))).toBe(true);
  });

  it("UnitDamaged — reduces energy first, then condition", () => {
    const { game, unitTypes } = makeApplyEventGame();
    const proc = new GameProcessor(game, unitTypes);
    proc.applyEvent({
      type: "UnitDamaged",
      unit: ref(1, 1), attacker: ref(2, 2),
      damageType: "Normal" as any, power: 8, damageToEnergy: 6, damageToCondition: 2,
    });
    const u = proc.getGame().players.get(1)!.units.get(uid(1))!;
    expect(u.energy).toBe(0);
    expect(u.condition).toBe(8);
  });

  it("UnitDamaged — does not go below zero", () => {
    const { game, unitTypes } = makeApplyEventGame();
    const proc = new GameProcessor(game, unitTypes);
    proc.applyEvent({
      type: "UnitDamaged",
      unit: ref(1, 1), attacker: ref(2, 2),
      damageType: "Normal" as any, power: 99, damageToEnergy: 6, damageToCondition: 93,
    });
    const u = proc.getGame().players.get(1)!.units.get(uid(1))!;
    expect(u.energy).toBe(0);
    expect(u.condition).toBe(0);
  });

  it("EnergyRegenerated — increases energy", () => {
    const { game, unitTypes } = makeApplyEventGame();
    const proc = new GameProcessor(game, unitTypes);
    // Drain some energy first
    proc.applyEvent({ type: "UnitDamaged", unit: ref(1, 1), attacker: ref(2, 2), damageType: "Normal" as any, power: 3, damageToEnergy: 3, damageToCondition: 0 });
    proc.applyEvent({ type: "EnergyRegenerated", unit: ref(1, 1), amount: 2, supported: false });
    expect(proc.getGame().players.get(1)!.units.get(uid(1))!.energy).toBe(5);
  });

  it("EnergyRegenerated — does not exceed maxEnergy", () => {
    const { game, unitTypes } = makeApplyEventGame();
    const proc = new GameProcessor(game, unitTypes);
    proc.applyEvent({ type: "EnergyRegenerated", unit: ref(1, 1), amount: 99, supported: false });
    expect(proc.getGame().players.get(1)!.units.get(uid(1))!.energy).toBe(6); // capped at maxEnergy
  });

  it("UnitDestroyed — removes unit from player", () => {
    const { game, unitTypes } = makeApplyEventGame();
    const proc = new GameProcessor(game, unitTypes);
    proc.applyEvent({ type: "UnitDestroyed", unit: ref(1, 1), destroyedBy: ref(2, 2) });
    expect(proc.getGame().players.get(1)!.units.has(uid(1))).toBe(false);
  });

  it("TurnStarted — updates currentPlayerId and turn", () => {
    const { game, unitTypes } = makeApplyEventGame();
    const proc = new GameProcessor(game, unitTypes);
    proc.applyEvent({ type: "TurnStarted", playerId: 2, turn: 2 });
    expect(proc.getGame().currentPlayerId).toBe(2);
    expect(proc.getGame().turn).toBe(2);
  });

  it("TurnStarted — records new turn start positions for valid move calculation", () => {
    const { game, unitTypes } = makeApplyEventGame();
    const proc = new GameProcessor(game, unitTypes);
    // Move p2 unit to (3,0) via event, then start p2 turn
    proc.applyEvent({ type: "UnitMoved", unit: ref(2, 2), position: createPosition(3, 0) });
    proc.applyEvent({ type: "TurnStarted", playerId: 2, turn: 2 });
    // p2 unit should have (3,0) as its turn-start position, so movement range is from there
    const validMoves = proc.getValidMovePositions(uid(2));
    expect(validMoves.some(p => p.q === 3 && p.r === 0)).toBe(true);
  });

  it("PlayerEliminated — marks player as eliminated", () => {
    const { game, unitTypes } = makeApplyEventGame();
    const proc = new GameProcessor(game, unitTypes);
    proc.applyEvent({ type: "PlayerEliminated", playerId: 2 });
    expect(proc.getGame().players.get(2)!.eliminated).toBe(true);
  });

  it("GameEnded — does not throw and leaves state intact", () => {
    const { game, unitTypes } = makeApplyEventGame();
    const proc = new GameProcessor(game, unitTypes);
    expect(() => proc.applyEvent({ type: "GameEnded", winnerId: 1 })).not.toThrow();
    expect(proc.getGame().players.size).toBe(2);
  });

  it("BuildOrdered — adds underConstruction unit at facility position", () => {
    const { game, unitTypes } = makeApplyEventGame();
    const proc = new GameProcessor(game, unitTypes);
    proc.applyEvent({ type: "BuildOrdered", unit: ref(99, 1), facilityPosition: createPosition(2, 0) });
    const newUnit = proc.getGame().players.get(1)!.units.get(uid(99))!;
    expect(newUnit).toBeDefined();
    expect(newUnit.underConstruction).toBe(true);
    expect(newUnit.position).toEqual(createPosition(2, 0));
    expect(newUnit.energy).toBe(6);   // maxEnergy from infantry
    expect(newUnit.condition).toBe(10); // maxCondition from infantry
  });

  it("UnitBuilt — clears underConstruction flag", () => {
    const { game, unitTypes } = makeApplyEventGame();
    const proc = new GameProcessor(game, unitTypes);
    proc.applyEvent({ type: "BuildOrdered", unit: ref(99, 1), facilityPosition: createPosition(2, 0) });
    proc.applyEvent({ type: "UnitBuilt", unit: ref(99, 1) });
    expect(proc.getGame().players.get(1)!.units.get(uid(99))!.underConstruction).toBeFalsy();
  });

  it("BuildCancelled — removes the underConstruction unit", () => {
    const { game, unitTypes } = makeApplyEventGame();
    const proc = new GameProcessor(game, unitTypes);
    proc.applyEvent({ type: "BuildOrdered", unit: ref(99, 1), facilityPosition: createPosition(2, 0) });
    proc.applyEvent({ type: "BuildCancelled", unit: ref(99, 1), facilityPosition: createPosition(2, 0) });
    expect(proc.getGame().players.get(1)!.units.has(uid(99))).toBe(false);
  });

  it("applyEvent round-trips: applying events emitted by handle() produces identical state", () => {
    const { game: g1, unitTypes } = makeApplyEventGame();
    const { game: g2 } = makeApplyEventGame();
    const source = new GameProcessor(g1, unitTypes);
    const mirror = new GameProcessor(g2, unitTypes);

    const events: GameEvent[] = [];
    source.handle({ type: "EndTurn" }, (e) => events.push(e));
    for (const e of events) mirror.applyEvent(e);

    const sg = source.getGame();
    const mg = mirror.getGame();
    expect(mg.currentPlayerId).toBe(sg.currentPlayerId);
    expect(mg.turn).toBe(sg.turn);
    // Both players' unit stats should match
    sg.players.forEach((sp, pid) => {
      const mp = mg.players.get(pid)!;
      expect(mp.eliminated).toBe(sp.eliminated);
      sp.units.forEach((su, uid) => {
        const mu = mp.units.get(uid)!;
        expect(mu).toBeDefined();
        expect(mu.energy).toBe(su.energy);
        expect(mu.condition).toBe(su.condition);
        expect(mu.position).toEqual(su.position);
      });
    });
  });
});
